import numpy as np


class Rollout:
    def __init__(self, env, max_steps):
        self.env = env
        self.max_steps = max_steps

    def __call__(self, policy, episodes):
        observation_size = self.env.observation_space.shape[-1]
        action_size = self.env.action_space.shape[-1]

        states = np.zeros(
            shape=(episodes, self.max_steps, observation_size),
            dtype=np.float32)
        actions = np.zeros(
            shape=(episodes, self.max_steps, action_size), dtype=np.float32)
        rewards = np.zeros(shape=(episodes, self.max_steps), dtype=np.float32)
        next_states = np.zeros(
            shape=(episodes, self.max_steps, observation_size),
            dtype=np.float32)
        weights = np.zeros(
            shape=(episodes, self.max_steps, observation_size), dtype=np.bool)

        for episode in range(episodes):
            state = self.env.reset()

            for step in range(self.max_steps):
                action = policy(state)
                next_state, reward, done, _ = self.env.step(action)

                states[episode, step] = state
                actions[episode, step] = action
                rewards[episode, step] = reward
                next_states[episode, step] = next_state
                weights[episode, step] = 1.0

                if done:
                    break

                state = next_state

        return states, actions, rewards, next_states, weights
